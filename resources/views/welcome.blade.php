...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'PresenceHub') }}</title>
        ...
        <div class="min-h-screen bg-gradient-to-b from-[#2f6aa8] via-[#2b5f98] to-[#214e83]">
            <header class="sticky top-0 z-50 bg-gradient-to-r from-[#0b2c52] to-indigo-700 text-white border-b border-white/10">
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="font-black tracking-tight">PresenceHub</span>
                    </div>

                    @if (Route::has('login'))
                        <div class="flex items-center gap-2">
                            @auth
                                <a href="{{ url('/dashboard') }}" class="inline-flex items-center justify-center px-5 py-3 rounded-2xl font-black text-xs uppercase tracking-[0.2em] bg-gradient-to-b from-indigo-600 to-indigo-700 text-white shadow-[0_14px_30px_rgba(79,_70,_229,_0.30)] ring-1 ring-indigo-600/20 transition">
                                    Dashboard
                                </a>
                            @else
                                ...
                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}" class="hidden sm:inline-flex items-center justify-center px-5 py-3 rounded-2xl font-black text-xs uppercase tracking-[0.2em] bg-gradient-to-b from-indigo-600 to-indigo-700 text-white shadow-[0_14px_30px_rgba(79,_70,_229,_0.30)] ring-1 ring-indigo-600/20 transition">
                                        Register
                                    </a>
                                @endif
                            @endauth
            ...
            <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
                    <div>
                        ...
                        <p class="mt-4 text-base font-medium text-white/80 leading-relaxed">
                            PresenceHub membantu pegawai melakukan presensi menggunakan pendeteksian lokasi dan foto, serta menyediakan monitoring dan laporan untuk pimpinan.
                        </p>
                ...
                <footer class="mt-14 pt-8 border-t border-white/15 text-center text-xs font-medium text-white/70">
                    © {{ date('Y') }} PresenceHub. All rights reserved.
                </footer>
            </main>
        </div>
    </body>
