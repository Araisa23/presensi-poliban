    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="mt-1 text-white/70 text-sm font-medium">Perbarui data pegawai tanpa mengubah akun user terhubung.</p>
            </div>
        </div>
    </x-slot>
                ...
                <form action="{{ route('admin.pegawai.update', $pegawai->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            <x-input-label for="user_id" :value="__('Akun User Connected')" />
                            <select id="user_id" name="user_id" class="block mt-2 w-full border-slate-200 dark:border-white/10 bg-slate-50/80 dark:bg-white/5 text-slate-900 dark:text-slate-100 rounded-2xl shadow-soft ring-1 ring-slate-900/5 dark:ring-white/10 transition opacity-80 pointer-events-none" required>
                                <option value="{{ $pegawai->user_id }}" selected>
                                    {{ $pegawai->user->name ?? 'User Tidak Ditemukan' }} ({{ $pegawai->user->email ?? '??' }})
                                </option>
                            </select>
                            <input type="hidden" name="user_id" value="{{ $pegawai->user_id }}">
                            <p class="text-xs text-amber-500 mt-1 italic">Akun user tidak dapat diubah di halaman ini demi integritas data.</p>
                        </div>
