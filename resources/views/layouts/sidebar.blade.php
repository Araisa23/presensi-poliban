<nav class="px-3 pt-6 pb-6 space-y-1">
       ...
       class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold {{ request()->routeIs('dashboard') ? 'bg-indigo-50 text-indigo-700 ring-1 ring-indigo-600/10' : 'text-slate-700 hover:bg-slate-50' }}">
        ...
        <a href="{{ route('admin.pengumuman.index') }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold {{ request()->routeIs('admin.pengumuman.*') ? 'bg-indigo-50 text-indigo-700 ring-1 ring-indigo-600/10' : 'text-slate-700 hover:bg-slate-50' }}">
            <span class="w-8 h-8 rounded-xl bg-white ring-1 ring-slate-900/10 flex items-center justify-center">
                <svg class="w-4 h-4 {{ request()->routeIs('admin.pengumuman.*') ? 'text-indigo-700' : 'text-slate-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5L6 9H2v6h4l5 4V5zM15.54 8.46a5 5 0 010 7.07M19.07 4.93a10 10 0 010 14.14"/></svg>
            </span>
            Pengumuman
        </a>
